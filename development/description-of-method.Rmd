...
**TODO** Finish up that function and transfer it back to tibble form and make a figure.
Also check for overlaps and remove them.
...
Consider a single locus with alleles $a_i, a_j,\ldots$. Let $n_{xyz}(a_ia_j)$ denote
the number of individuals in which this locus occurs within an $A_xB_yC_z$ segment
and at which the genotype of the individual at the locus is $a_ia_j$. 
...
$$
In fact, the only scenarios that are ambiguous about how many $a_i$ alleles are on
...
the E-step (I think---I always have a hard time figuring out which is which...).
$$
\begin{aligned}
