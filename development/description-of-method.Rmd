...
the E-step (I think---I always have a hard time remembering which is the E and which is the M step).
$$
\begin{aligned}
...
gametes that could be segregated by the parent. To understand this, it is worth considering
all the different possibilities.  Let $H_1$ and $H_2$ denote two distinct ancestries/subspecies.
Note that withing a single individual, you will never find more than two.  Likewise, we will
let $V_1$ and $V_2$ denote two distinct alleles at a variable marker.  We can then describe
an individuals genotype and ancestry like this, for example: $V_1V_1,H_1H_2$, which denotes
an individual homozygous for an allele and heterozygous for ancestry.  Then, the possible,
canonical, combinations of alleles and ancestries, grouped by the number of possible
gametes that they will segregate are:

- Those that will yield only one possible type of gamete:
    - $V_1V_1,H_1H_1$
- Those that can yield two distinct possible gametes:
    - $V_1V_2,H_1H_1$
    - $V_1V_1,H_1H_2$
- Those that can yield four possible gametes:
    - $V_1V_2,H_1H_2$
