...
We step back here and sketch this out in mathematical form.  Let $Y_P$ and $Y_C$ be the
genotypes at the variable markers within compatible intersected intervals of the
candidate parent ($P$) and the candidate offspring ($C$). Then the joint probability of
their genotypes will look something like this:
$$
\begin{aligned}
...
Note that within a single individual, you will never find more than two.  
...
an individuals genotype as $V_1V_1$ or $V_1V_2$.  In the above, where we talk about
...
\end{aligned}
$$
where $\epsilon$ is essentially the rate at which alleles are miscalled, in a very
simple genotyping error model.  

Great! All I need now is a little pseudo code for the products and sums above
and I will be ready to start implementing this.  I will do it using RCpp.
