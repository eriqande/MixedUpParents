...
ggsave(SDM_plot, filename = "stacked_segs.pdf", width = 10, height = 30)
...
AlleFreqs <- afem_list$alle_freqs
GenoCounts <- afem_list$geno_counts
```


The output of AlleFreqs is a little complex:
```{r}
head(AlleFreqs)
```
...
observed_geno_freqs <- GenoCounts %>%
  filter(copy_num %in% c("A2B0C0", "A1B1C0", "A0B2C0")) %>%
  complete(
...
as bad as $|z| > 3.09$ (which gives a cumulative prob of 1 out of 1000; so, it is like
a p-value of 0.001) within the copy_num at a site.

```{r}
exp_and_obs_geno_freqs_3 <- exp_and_obs_geno_freqs_2 %>%
  group_by(chrom_f, pos, copy_num) %>%
  filter(all(abs(z_score) < 3.09))
...
So, those are pretty decent numbers of markers that we will have to work with, I think.
...
Let's keep the SNPs with a MAF > 0.10.

```{r}
retained_freqs_maf_gt10 <- retained_freqs2 %>%
  filter(MAF > 0.1) %>%
  select(chrom_f, pos, Ancestry, MAF)

# now we have to get the indivdiuals on there but only for those markers.
V %>%
  mutate(chrom_f = factor(chrom, levels = levels(retained_freqs_maf_gt10$chrom_f))) %>%
  select(-var_spp, -chrom)
```
...
First thing, let's make the integer representation of of E:
```{r}
...
```{r}
spoing <- intersect_ancestry_intervals_rcpp(
  grp1 = 0,
...
Now, what we need to do is add a way to quickly calculate the probability of the genotypes (and hence the likelihood ratio)
